...
const TEMPLATES = {
  '남자': 'KA01TP250821132255320m0JTlRlILRm',
  '여자': 'KA01TP250821132255320m0JTlRlILRm',
  'DEFAULT': 'KA01TP250821132255320m0JTlRlILRm' // 성별이 없거나 일치하지 않을 때 사용할 기본값
};

const MEMO_FIELD = 'memo3'; // 상태를 기록할 DB 필드
const KEYWORD = '입금재촉'; // 중복 발송을 막기 위한 키워드
...
module.exports = {
  ...
  getUsers: async (supabase, ids) => {
    ...
    const { data, error } = await supabase
      ...
      .in('id', ids)
      .or(
        `${MEMO_FIELD}.not.like.%${KEYWORD}%,` +     // 1. '입금재촉'가 포함되지 않았거나
        `${MEMO_FIELD}.like.%❌${KEYWORD}%,` +      // 2. '❌입금재촉'가 포함되었거나
  ...
  getTemplateId: (user) => {
    ...
    console.log(`[reminder] ${user.name}님(${user.gender})에게 '${templateId}' 템플릿을 사용합니다.`);
  ...
  getVariables: (user) => {
    ...
    //   '#{고객명}': user.name,
    //   '#{파티명}': '게릴라 파티',
    //   '#{date}': formatKoreanDate(user.apply_date)
    // };
    return {}; // 현재는 빈 객체 반환
  },
